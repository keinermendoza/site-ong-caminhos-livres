...
function validateEmail(emailfield){const email=emailfield.value.trim();const emailPattern=/^[a-z0-9._%+\-]+@[a-z0-9.\-]+\.[a-z]{2,}$/;if(!emailPattern.test(email)){setError(emailfield,"E-mail inválido. Por favor, verifique o formato do e-mail")}}
...
function validateCPF(cpfField){if(!coreValidateCPF(cpfField)){setError(cpfField,"CPF inválido.")}}
...
cidade.value=data.localidade;estado.value=data.estado}catch(e){setError(cepField,`Não foi possivel reconhecer o cep ${cep}`)
